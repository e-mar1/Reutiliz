...
<nav class="flex items-center space-x-2">
    ...
    @auth
        <div x-data="{ open: false }" class="relative">
            ...
            <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-md shadow-lg z-50 py-1">
                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-xs text-gray-700 hover:bg-blue-50">Profil</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-left block px-4 py-2 text-xs text-gray-700 hover:bg-blue-50">Déconnexion</button>
                </form>
            </div>
        </div>
    @else
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                        @foreach($items as $item)
                            <article class="bg-white rounded-xl shadow hover:shadow-lg transition-all duration-200 flex flex-col overflow-hidden">
                                <a href="{{ route('components.show-details', $item->id) }}">
