            ...
            <div class="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
                ...
                <div x-show="mobileMenuOpen" @click.away="mobileMenuOpen = false" class="sm:hidden border-t border-gray-200 bg-white shadow-md rounded-b-xl">
                    <div class="px-4 py-3 space-y-2">
                        <a href="{{ route('items.create') }}" class="block px-3 py-1 rounded-md text-xs font-medium text-white bg-blue-600 hover:bg-blue-700"><i class="fas fa-plus-circle mr-1"></i>Publier</a>
                        @auth
                        ...
                        @foreach($items as $item)
                            <article class="bg-white rounded-xl shadow hover:shadow-lg transition-all duration-200 flex flex-col overflow-hidden">
                                <a href="{{ route('items.show', $item) }}" class="block bg-gray-100 aspect-[4/3] overflow-hidden">
                                    @if($item->image)
                                        <img src="{{ asset('storage/items/' . $item->image) }}" alt="{{ $item->title }}" class="w-full h-full object-cover object-center transition-transform duration-200 hover:scale-105">
                                    @else
                                ...
                                <div class="p-4 flex flex-col flex-grow">
                                    ...
                                    <h3 class="text-base font-semibold text-gray-900 mb-1 leading-tight">
                                        <a href="{{ route('items.show', $item) }}" class="hover:text-blue-600 transition-colors">{{ Str::limit($item->title, 45) }}</a>
                                    </h3>
        ...
        <footer class="bg-gray-900 text-gray-300 mt-10">
            <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
                ...
                <div class="mt-10 border-t border-gray-700 pt-6 text-center text-xs text-gray-400">
                    © {{ date('Y') }} Reutiliz. Tous droits réservés.
                </div>
            </div>
        </footer>
    </div>
